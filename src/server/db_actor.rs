...
// It's possible to use the actix-derive crate to eliminate some boilerplate here, but not in
// this case, because these types are code-generated by tower-grpc.
...
impl Handler<ScanRequest> for SledExecutor {
    ...
    fn handle(&mut self, msg: ScanRequest, _: &mut Self::Context) -> Self::Result {
        ...
        // range returns an Iterator of Result. I don't want to deal with trying to map that into
        // gRPC messages for this demo, so I'm mapping the Result to an Option, then filtering out
        // None to build a Vec<Pair>.
